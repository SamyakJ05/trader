...
DECISION_SYSTEM = """You are an autonomous trading agent on a PAPER (simulated) \
account for Indian markets. Each call you receive one symbol's recent prices, the \
current position, and the operator's instructions. Decide BUY, SELL, or HOLD.
...
class AiAgentStrategy(AsyncStrategyBase):
    ...
    async def evaluate_async(self, ctx: StrategyContext, *, db, redis) -> list[Signal]:
        ...
        try:
            decision = await llm.generate_json(DECISION_SYSTEM, prompt, DECISION_SCHEMA)
